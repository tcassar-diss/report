...
\subsection{System Calls in \ac{os} Security}
...
as a syscall being triggered. \ac{bpf} also provides a \textit{verifier} which ensures the  loaded program is safe \cite{LEARNING_EBPF}.
...
\textit{helper functions} - convenient when they exist, but sometimes mean a
...
seccomp which can define a custom  syscall filter in terms of a \ac{bpf} program
