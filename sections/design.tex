...
program we've written is not interfering with normal system call functionality. Then,
we test that our filtering system works - that is, \af intervenes if and only if
a system call that is not on the whitelist is made.

To make sure that \af does not interfere with normal system call functioning, we
propose to use the \acg{ltp} system call test suite. We will run the suite on our
...
system call filtering works as expected. Writing a comprehensive test harness in requires 
...
system calls and dynamically generate whitelists (as discussed in
\ref{subsubsec:additional-tooling}). Using a restricted set of system calls allows
...
application. To do this, we define a \textit{privilege metric} (Equation~\ref
{eqn:privilege}) and compare how privileged seccomp-filtered applications
are vs \afss-filtered applications.

We then look to evaluate how costly \af is at runtime. To do this, we will take
a broad array of benchmarks informed by modern system call filtering literature and
run them with \af enabled. We will also run the same set of system calls with a
