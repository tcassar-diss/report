...
% TODO: Cite that bpf programs can be attached without "sidecars"
% TODO: Cite that kernel modules require recompilation
...
\begin{figure}[ht]
  ...
  \begin{tikzpicture}[node distance=1cm, auto]
    \node[tech]              (golang) {Go};
    \node[tech, below=of golang] (ebpfgo) {eBPF-Go};
    \node[tech, below=of ebpfgo]  (bpf)    {eBPF};
    \draw[->, thick] (bpf)    -- (ebpfgo);
    \draw[->, thick] (ebpfgo) -- (golang);
  \end{tikzpicture}
...
% TODO: cite kernel helper functions docs
% TODO: cite functions are unrepresentable in BPF
...
% TODO: cite pointer based data structures slow
...
\todo{add figure showing reading the relevant structs}
...
\todo{flowchart/swimlane showing intervention strat for each config mode}
...
is discarded.
% TODO: cite ktime is monotonic, and that normal clocks are bad for timing.

% TODO: profiling flow diagram
%   profiling info gathered during execution
%   written to ringbuffer (or discarded if an error occurs during execution)
%   read on frontend
%   diffs calculated
%   written to exec-profile.csv
\todo{profiling flow diagram}
...
\todo{frontend execution flow diagram}
...
\todo{check whether processes or users have the CAP\_SYS \_ADMIN privlege}
...
\todo{check this is true! if prctl in whitelist then definitely true. Can prctl
be used for anything else? can anything else be used to (un)mount bpf programs?}
...
\todo{change the phrasing: cannot pass an interface!}
...
\todo{how to justify the \texttt{syso} design decisions? theyre silly because i
did them first!}
